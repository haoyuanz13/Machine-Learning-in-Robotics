...
def main_cartpole(n_iter=100, gamma=1.0, min_timesteps_per_batch=1000, stepsize=1e-2, animate=True, logdir=None):
    ...
    # use output from the last layer to compute mean and variance
    logstdev = 0.1
    mean_na = dense(sy_logits_na, 1,'mean', weight_init=normc_initializer(0.1))  # Mean control output
    logstd_a = tf.get_variable("logstdev", [1], initializer=tf.zeros_initializer,dtype=tf.float32)  # Variance

    Gaus = tf.contrib.distributions.Normal(mu=mean_na, sigma=tf.exp(logstd_a))
    sy_sampled_ac = Gaus.sample([1])
    sy_logprob_n =  Gaus.prob(sy_sampled_ac)
    ...
    sy_logp_na = Gaus.prob(mean_na) # log probability of actions
    ...
    for i in range(n_iter):
        ...
        # Policy update
        _, oldlogits_na = sess.run([update_op, mean_na], feed_dict={sy_ob_no:ob_no, sy_ac_n:ac_n, sy_adv_n:standardized_adv_n, sy_stepsize:stepsize})
