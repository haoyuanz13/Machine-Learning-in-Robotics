...
from skimage import measure, draw, data, segmentation, measure, morphology, color
...
def label_redB(ma, threshold):
  ...
  mask = (sum_prob_rb > threshold) & (sum_prob_rb > sum_prob_rc) & \
         (sum_prob_rb >= 0.05 * sum_prob_ro1) & \
         (sum_prob_rb >= 0.12 * sum_prob_ro2)
...
def pos_barrel(t):
  ...
  thresh = filter.threshold_otsu(t)
...
'''
def main(folder):
  global dic_rb, dic_rc, dic_ro1, dic_ro2
  ...
  global cluster_rb, cluster_rc, cluster_ro1, cluster_ro2
...
if __name__ == "__main__":
  # folder directory to store all test images
  test_path = "test"
  main(test_path)
